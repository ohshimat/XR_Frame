...
void drawImage(image_t *image, int x, int y, bool alpha = true );
...
public:
	...
	ezImage( const char *file ){
		if (strstr(file, ".png") != nullptr ||
			strstr(file, ".PNG") != nullptr) {
			image = newImagePng(file);
		}
		else {
			image = newImageJpg(file);
		}
		meshid = makeMesh( 0, true );
		allocTexture(image);
	}
	~ezImage(){
...
public:
	...
	void copy(unsigned char *srcimage) { //※未テスト
