...
//------------------------------------------------------------
image_t *newImagePng(const char *file)
{
	image_t *image = newImage(0, 0, GL_RGBA);/////////////OKOKOKOK
	image->pixdata = readImagePNG(file, &image->width, &image->height);
	return image;
}
//------------------------------------------------------------
int loadImagePng(image_t *image, const char *file) 
{
	int w, h;
	image->pixdata = readImagePNG(file, &w, &h, (unsigned char*)image->pixdata);
	image->width = w;
	image->height = h;
	if (w != image->width || h != image->height) {
		return -1;
...
//------------------------------------------------------------
void saveImagePng(image_t *image, const char *file) 
{
	writeImagePNG(file, (unsigned char*)image->pixdata, image->width, image->height);
}
//------------------------------------------------------------
void drawImage(image_t *image, int x, int y, bool alpha)
{
	//////// ALPHA EFFECT
	bool stencil = false;
...
void allocTexture(image_t *image)
{
	...
	*/
	...
	glTexImage2D(GL_TEXTURE_2D, 0, image->pixformat, image->width, image->height, 0, image->pixformat,
		GL_UNSIGNED_BYTE, image->pixdata);
...
void drawMesh( int texid, int meshid )
{
	...
	//glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE); //REPLACE
