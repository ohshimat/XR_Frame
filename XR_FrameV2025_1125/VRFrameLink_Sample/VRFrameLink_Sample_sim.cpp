...
{
	if (VRFL::OBJCreateModel(filename, 1.0)) {
		printf("OBJモデル\"%s\"の読み込みに成功しました。\n", filename);
	}
	else {
		printf("OBJモデル\"%s\"の読み込みに失敗しました。\n", filename);
	}
    ...
    for (int i = 0; i < materialCount; i++) {
        ...
        int hasAmbient, hasDiffuse, hasSpecular, hasTransparency, hasShininess, hasIllumination, hasTexture, hasSphereTexture, hasCubeTexture;
        float ambient[3], diffuse[3], specular[3], transparency;
        int shininess, illumination, textureID, sphereTextureID, cubeTextureIDs[6];
        if (VRFL::OBJMaterialInfo(i,
			...
			if (hasTexture) {
				...
				if (VRFL::OBJTextureBufferInfo(textureID,
					&textureWidth, &textureHeight,
					&textureDepth, &textureBufferSize)) {
					printf("    テクスチャサイズ: %dx%d (depth=%d)\n", textureWidth, textureHeight, textureDepth);
					printf("    テクスチャバッファサイズ: %d\n", textureBufferSize);

					unsigned char* textureBuffer = new unsigned char[textureBufferSize];
					if (VRFL::OBJTextureBufferImage(textureID, textureBuffer, textureBufferSize)) {
						printf("    テクスチャバッファの画像データを取得しました。\n");
					}
					else {
						printf("    テクスチャバッファの画像データの取得に失敗しました。\n");
					}
					delete[] textureBuffer;
				}
				else {
					printf("    テクスチャバッファ情報の取得に失敗しました。\n");
				}
			}
            printf("  球面テクスチャ: %s\n", hasSphereTexture ? "あり" : "なし");
            if (hasSphereTexture) {
                printf("    球面テクスチャID: %d\n", sphereTextureID);
            }
            printf("  立方体テクスチャ: %s\n", hasCubeTexture ? "あり" : "なし");
            if (hasCubeTexture) {
                printf("    立方体テクスチャIDs: ");
                for (int j = 0; j < 6; j++) {
                    printf("%d ", cubeTextureIDs[j]);
                }
                printf("\n");
            }
        }
...
void InitScene(void)
{
	...
	VRFL::OBJInit();
    LoadOBJ("../objmodel/iltj/Box.obj");
    LoadOBJ("../objmodel/heart/test_0.obj");
    LoadOBJ("../objmodel/kuma/kuma.obj");
	VRFL::OBJCleanUp();
