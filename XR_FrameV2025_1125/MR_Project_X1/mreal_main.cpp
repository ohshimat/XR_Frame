...
BOOL initMrPlatform() {
	...
	// エンジンインスタンスを作成
	HRESULT hr = S_OK;
	hr = CoCreateInstance(CLSID_CMrhEngine62, NULL, CLSCTX_INPROC, IID_IMrhEngine62, (LPVOID*)&pEngine);
	...
	if (isExist) {
		...
		pEngine->GetRenderer60(MRH_CAMERA_MASTER, &pRender[dwNumOfCameras]); //////// 53
		dwNumOfCameras++;
	}
	// スレーブカメラがあるかどうか確認
	pEngine->QueryCamera(MRH_CAMERA_SLAVE, &isExist);
	if (isExist) {
		...
		pEngine->GetRenderer60(MRH_CAMERA_SLAVE, &pRender[dwNumOfCameras]); //////// 53
		dwNumOfCameras++;
	}
	...
	if (dwNumOfCameras == 2) {
		...
		res = pEngine->GetDepth70(MRH_DEPTH_COLOR_REGION, &depth); //////// 53
